...
const previous = globalThis.__worktimeLayout;

const roleOrder = { MEMBER: 1, OPERATOR: 2, MASTER: 3 };

const isLinkAllowed = (link, role) => {
...
const isPageAllowed = (activePage, role) => {
...
const showWeakPasswordPrompt = (user) => {
...
const initAppLayout = previous?.initAppLayout || (async (activePage) => {
  showAppShellLoader();
...
const initLoginShell = previous?.initLoginShell || (async () => {
  setupSidebar();
...
export { initAppLayout, initLoginShell };
